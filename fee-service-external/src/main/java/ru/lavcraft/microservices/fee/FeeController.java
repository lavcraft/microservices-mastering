...
public class FeeController {
  ...
  public FeeResponse fee(@RequestHeader("Authorization") String tokenHeader, @RequestBody FeeRequest request) {
    log.info("fee request={}", request);

    String token  = Optional.ofNullable(tokenHeader)
        .orElseThrow(() -> new RuntimeException("Token not presented"));
...
//            .build()
        userServiceClient.getTUser(token),
        paymentServiceClient.getPaymentInfoForUser(1L, request.getOperationType()),
